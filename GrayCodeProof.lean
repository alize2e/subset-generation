      ...
      if i.succ<((grayRecSlides n').map (cons false)).length then
        ...
        if i.succ = ((grayRecSlides n').map (cons false)).length then
          ...
          calc num_changes (grayRecSlides n'.succ)[i] (grayRecSlides n'.succ)[i.succ]
            ...
            _ = num_changes (grayRecSlides n')[(grayRecSlides n').length-1-(i-(grayRecSlides n').length)] (grayRecSlides n')[(grayRecSlides n').length-1-(i.succ-(grayRecSlides n').length)] := sorry -- by simp [List.sorry_getElem_reverse] -- FIX!!!!!
