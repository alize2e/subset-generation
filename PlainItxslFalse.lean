-- adds proof and param that xsl is made entirely of zeros
...
def Subset.genIt (n : Nat) : List (Subset n) :=
  let rec help {l r : Nat} (xsl : Subset l) (xsr : Subset r) (soFar : List (Subset n))
    (sum_l_r_n : l+r=n) (zeros : xsl.binValOne 0 = 0) (nel : soFar.length>0)
    (xsl_false : xsl = initFalse l): List (Subset n) :=
      match xsr with
      | Subset.nil => soFar
      | Subset.cons x xsr' =>
          if h:x then -- xsl is all 0s
            ...
            have zeros' : xsl'.binValOne 0 = 0 :=
              calc xsl'.binValOne 0
                ...
                _ = 0 := by simp [zeros]
            have xsl_false' : (cons false xsl) = initFalse (cons false xsl).card :=
              calc (cons false xsl)
                _ = cons false (initFalse xsl.card) := by rw [xsl_false]
                _ = initFalse (cons false xsl).card := by rfl
            help xsl' xsr' soFar sum_l'_r'_n zeros' nel xsl_false'
          else
            ...
            help Subset.nil (xs) ((Subset.cast (by simp [xs_len_eq_n]) xs) :: soFar) sum_l'_r'_n rfl (by simp [nel]) rfl
      termination_by (soFar[0].binValOne 1, binVal xsl xsr 0)
      -- xsl'++xsr' = incr xsl++xsn (where lsb is left)

  let init := initFalse n
  help Subset.nil init ([init]) (by simp_arith) rfl (by simp) rfl
